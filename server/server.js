...
io.on('connection', function (socket) {
    ...
    socket.on('gotit', function (player) {
        ...
        if (findPlayer(player.id) === null) {
            console.log('Player ' + player.id + ' connected using name: ' + player.name);
            users.push(player);
            currentPlayer = player;
            console.log("CPU Average For Last [Minute], [5 Minutes], [15 Minutes]: " + os.loadavg());
            currentTime = new Date;
            console.log("Server has been up for: " + (((currentTime.getTime() - startTime.getTime())/1000)/60) + " Minutes!");
        }
    ...
    socket.on('cole', function () {
        socket.emit('sendToCole');
    });
	...
	if(playerDisconnected.hasOwnProperty('name')){
        ...
        console.log("Players Online: " + users.length);
